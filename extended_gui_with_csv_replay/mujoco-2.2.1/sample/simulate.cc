...
// Struct to hold in the csvData
struct csvData{
  string jointName;
  vector<double> numberVector;
};

// Vector to store csvData structs
vector<csvData> csvDataSet; 

//-------------------------------- global -----------------------------------------------

bool playToggle = false;
bool halfSpeedSwitch = false;

static constexpr int kBufSize = 1000;

int jointMovements = 0;
...
// Vector to hold the joints used for movement
vector<csvData> usedJoints;
map<string, vector<double>> dynamicVariables;
...
struct {
  ...
  int sld_time = 0; // slider time in seconds
  int sld_speed = 0;
...
const mjuiDef defOption[] = {
  {mjITEM_SECTION,   "Option",        0, NULL,                    "AO"},
...
// Extended UI section
const mjuiDef defProjectile[] = {
  {mjITEM_SECTION, "CSV Replay", 1, NULL, ""},
  {mjITEM_SLIDERINT, "Time Slider", 2, &settings.sld_time,  "0 3261"},
  {mjITEM_SLIDERINT, "Play Speed", 2, &settings.sld_speed,  "0 16"},
  {mjITEM_BUTTON, "<<", 1, NULL, ""},
  {mjITEM_BUTTON, ">>", 1, NULL, ""},
  {mjITEM_BUTTON, "Play", 1, NULL, ""},
  {mjITEM_BUTTON, "Stop", 1, NULL, ""},
  {mjITEM_BUTTON, ".5x Speed", 1, NULL, ""},
  {mjITEM_END}
};


// simulation section of UIF
...
void sensorinit(void) {
  ...
  // title
  mju::strcpy_arr(figsensor.title, "X Position");
  ...
  figsensor.range[1][0] = -5;
  figsensor.range[1][1] = 5;
}
...
void sensorupdate(void) {
  ...
  for (int n=0; n<m->nsensor; n++) {
    ...
    for (int i=0; i<dim; i++) {
      ...
      figsensor.linedata[lineid][2*p+4*i+3] = dynamicVariables.at("dwa_x_ref").at(settings.sld_time);
    }
...
void sensorshow(mjrRect rect) {
  ...
  int width = settings.profiler ? rect.width/4 : rect.width/4;
...
void updateModel(){
            ...
            for (int i = 4; i < nact + 4; i++){
              d->qpos[q_id[i-4]] = usedJoints.at(i).numberVector.at(settings.sld_time); //sets the joints position row by row
            }
            
            d->qpos[7*(com_id_body-2)+0] = dynamicVariables.at("dwa_x_ref").at(settings.sld_time); //should move x
            d->qpos[7*(com_id_body-2)+1] = dynamicVariables.at("dwa_y_ref").at(settings.sld_time); //should move y
            d->qpos[7*(com_id_body-2)+2] = dynamicVariables.at("WorldPosZ").at(settings.sld_time); //should move z
            
            d->qpos[7*(com_id_body-2)+3] = dynamicVariables.at("q0").at(settings.sld_time); // q0
            d->qpos[7*(com_id_body-2)+4] = dynamicVariables.at("qx").at(settings.sld_time); // qx
            d->qpos[7*(com_id_body-2)+5] = dynamicVariables.at("qy").at(settings.sld_time); // qy
            d->qpos[7*(com_id_body-2)+6] = dynamicVariables.at("qz").at(settings.sld_time); // qz
...
int countJoints(const string& firstLine){
  // Counts commas in the first line to get the number of joints
  int count = 0;
...
void jointFilter(vector<csvData>& usedJoints, const int numJoints, vector<string>& wantedJoints){
  // Loop to get those specific joints from the entire dataset
  for(int i = 0; i < numJoints; i++){
    for(int j = 0; j < wantedJoints.size(); j++){
      if(csvDataSet.at(i).jointName == wantedJoints.at(j))
        usedJoints.push_back(csvDataSet.at(i));
      }
  }
}
...
void uiEvent(mjuiState* state) {
  ...
  if ((state->dragrect==ui0.rectid) ||
    ...
    // Extended GUI
    else if (it && it->sectionid==SECT_PROJECTILE) {
      switch (it->itemid) {
        case 0:
        break;
        case 1:
            playToggle = false;
         break;
        case 2: // Button 1 (go backwards one frame)
          if(settings.sld_time > 1){
            settings.sld_time--;
            settings.sld_time--;
            mjui_update(-1, -1, &ui0, &uistate, &con);
            sensorupdate();
          }else if(settings.sld_time > 0){
            settings.sld_time--;
            mjui_update(-1, -1, &ui0, &uistate, &con);
            sensorupdate();
          }
          ...
          break;
        case 3: // Button 2 (go forwards one frame)
          if(settings.sld_time < jointMovements-3){
            settings.sld_time++;
            settings.sld_time++;
            sensorupdate();
            mjui_update(-1, -1, &ui0, &uistate, &con);
          }else if(settings.sld_time < jointMovements-2){
            settings.sld_time++;
            mjui_update(-1, -1, &ui0, &uistate, &con);
            sensorupdate();
          }
        ...
        if(!halfSpeedSwitch){
          while(playToggle == true){
            if(settings.sld_time < jointMovements-2){
                if(settings.sld_time < jointMovements-settings.sld_speed-2){
                    for(int i=0; i<settings.sld_speed; i++){
                        settings.sld_time++;
                    }
                }else{
                    while(settings.sld_time < jointMovements-2){
                        settings.sld_time++;
                        updateModel();
                        mjui_update(-1, -1, &ui0, &uistate, &con);
                        sensorupdate();
        ...
        case 5: // Button 4
          ...
          break;
        case 6: // Button 5
          halfSpeedSwitch = true;
          playToggle = true;
          while(settings.sld_time < jointMovements-2 && playToggle == true){
            settings.sld_time++;
            updateModel();
            mjui_update(-1, -1, &ui0, &uistate, &con);
            sensorupdate();
...
int main(int argc, const char** argv) {
  ...
  ifstream file(path1);
    ...
    if(file.is_open()){
    // Calls the function to count the number of joints in the first line
    numJoints = countJoints(firstLine);
      for(int i = 0; i < numJoints; i++){
        // Makes a new piece of data for every joint
        csvData data;
        // Finds where the comma is in the first line where the joints are listed
        commaPos = firstLine.find_first_of(',');
        // Sets the name of the joint according to the commas position
        data.jointName = firstLine.substr(0, commaPos);
        // Removes the name from the firstLine variable to prepare the next one to be included
        firstLine.erase(0, commaPos + 1);
        // Adds the struct to the dataset
        csvDataSet.push_back(data);
      }
      
      while (getline(file, currLine)){
        for(int i = 0; i < numJoints; i++){
        // Finds where the first comma is in the data, and sets commaPos to there
        commaPos = currLine.find_first_of(',');
        // Adds the data from the first comma (the time) to the specific joint its for
        csvDataSet.at(i).numberVector.push_back(stod(currLine.substr(0, commaPos)));
        // Removes the value that was for the previous data point to prepare the next one
        currLine.erase(0, commaPos + 1);
        }
      }
    }
    ...
    //vector to hold the joints that are wanted to be used for the movement for the function to sort into usedJoints
    vector<string> wantedJoints = {"q_FR_hip", "q_FR_thigh", "q_FR_calf", "q_FL_hip", "q_FL_thigh", "q_FL_calf" , "q_RR_hip", "q_RR_thigh", "q_RR_calf", "q_RL_hip", "q_RL_thigh", "q_RL_calf", "com_x", "com_y" , "com_z", "q0", "qx", "qy", "qz"};
    jointFilter(usedJoints, numJoints, wantedJoints);

    jointMovements = countLines();
    int increment = 0;
    
    for(int i = 0; i < csvDataSet.size(); i++){
      dynamicVariables[csvDataSet.at(i).jointName] = csvDataSet.at(i).numberVector;
    }
